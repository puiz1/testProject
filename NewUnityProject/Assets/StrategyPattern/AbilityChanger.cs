...
namespace DesignPatterns.StrategyPattern
{
    ...
    public class AbilityChanger : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.F) && !(currentAbility is FireBallAbility))
                m_abilityRunner.CurrentAbility = new FireBallAbility();
            if (Input.GetKeyDown(KeyCode.H) && !(currentAbility is HealAbility))
                m_abilityRunner.CurrentAbility = new HealAbility();
            if (Input.GetKeyDown(KeyCode.M) && !(currentAbility is MeleeAbility))
                m_abilityRunner.CurrentAbility = new MeleeAbility();
            if (Input.GetKeyDown(KeyCode.R) && !(currentAbility is RageAbility))
